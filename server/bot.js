...
const INDIAN_CITIES = [
  ...
  'Jaipur', 'Surat', 'Lucknow', 'Kanpur', 'Nagpur', 'Indore', 'Bhopal', 'Vadodara',
  'Patna', 'Ranchi', 'Raipur', 'Guwahati', 'Amritsar', 'Ludhiana', 'Jalandhar', 'Chandigarh',
  'Gurgaon', 'Noida', 'Ghaziabad', 'Faridabad', 'Meerut', 'Agra', 'Mathura', 'Aligarh',
  'Bareilly', 'Moradabad', 'Prayagraj', 'Varanasi', 'Gorakhpur', 'Jabalpur', 'Gwalior',
  'Ujjain', 'Ratlam', 'Aurangabad', 'Nashik', 'Thane', 'Panaji', 'Margao', 'Mangaluru',
  'Udupi', 'Hubballi', 'Belagavi', 'Madurai', 'Salem', 'Erode', 'Thanjavur', 'Tiruppur',
  'Vellore', 'Nellore', 'Vijayawada', 'Guntur', 'Warangal', 'Nizamabad', 'Karimnagar',
  'Kakinada', 'Rajahmundry', 'Visakhapatnam', 'Cuttack', 'Rourkela', 'Durgapur', 'Asansol',
  'Howrah', 'Siliguri', 'Agartala', 'Imphal', 'Aizawl'
];
...
function isConnectionError(error) {
  const connectionErrors = [
    ...
    'ERR_ADDRESS_UNREACHABLE', 'net::ERR_', 'Protocol error', 'Target closed',
    'Session closed', 'Navigation timeout', 'timeout', 'unreachable',
    'ECONNREFUSED', 'ECONNRESET', 'ETIMEDOUT', 'ENOTFOUND', 'closed', 'disconnected'
  ];
  const errorString = error.toString().toLowerCase();
  return connectionErrors.some(errType => errorString.includes(errType.toLowerCase()));
}
...
async function runLoop(loopNum) {
  updateState({ 
    currentLoop: loopNum, 
    currentStep: `Loop ${loopNum}/20 - Starting` 
  });
  saveData(LOOP_FILE, { 
    currentLoop: loopNum, 
  ...
  try {
    updateState({ currentStep: `Loop ${loopNum}/20 - Step 1: Visiting website` });
    await safeGoto('https://territorial.io');
    await delay(5000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 2: Refreshing` });
    await safeReload();
    await delay(1000);
    ...
    updateState({ currentStep: `Loop ${loopNum}/20 - Step 4: Clicking Multiplayer` });
    await safeClick(349, 297, 'Multiplayer');
    await delay(5000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 5: Clicking Team Mode` });
    await safeClick(84, 55, 'Team Mode');
    await delay(2000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 6: Clicking Ready` });
    await safeClick(590, 566, 'Ready');
    await delay(60000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 7: Playing (8 minutes)` });
    ...
    updateState({ currentStep: `Loop ${loopNum}/20 - Step 8: Menu` });
    await safeClick(18, 579, 'Menu');
    await delay(1000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 9: Exit` });
    await safeClick(18, 579, 'Exit');
    await delay(5000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 10: Close` });
    await safeClick(18, 579, 'Close');
    await delay(3000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 11: Home` });
    await safeClick(18, 579, 'Home');
    await delay(1000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 12: Settings` });
    await safeClick(449, 363, 'Settings');
    await delay(5000);

    updateState({ currentStep: `Loop ${loopNum}/20 - Step 13: Replay` });
    ...
    updateState({ currentStep: `Loop ${loopNum}/20 - Step 16: Delay` });
    await delay(10000);
  ...
  try {
    ...
    while (botState.running) {
      try {
        ...
        console.log(`\n${'='.repeat(60)}`);
        console.log(`COMPLETED CYCLE #${botState.totalCycles}`);
...
// Auto-start the bot when script is run
console.log('='.repeat(60));
console.log('TERRITORIAL.IO BOT - AUTO-STARTING');
console.log('='.repeat(60));
console.log('Bot will run continuously. Press Ctrl+C to stop.');
console.log('='.repeat(60));
